...
fn run() -> Result<(), String> {
    ...
    for edge in snapshot.edges {
        ...
        total_metadata_bytes = total_metadata_bytes.saturating_add(metadata_byte_len(&normalized.metadata));
        edges.push(normalized);
    }
    if total_metadata_bytes > MAX_TOTAL_METADATA_BYTES {
        return Err(format!(
            "total metadata size too large: {total_metadata_bytes} bytes (maximum {MAX_TOTAL_METADATA_BYTES} bytes)"
        ));
    }
    if edges.is_empty() {
...
fn normalize_edge(edge: Edge) -> Result<Edge, String> {
    ...
    if let Some(map) = normalized.metadata.as_object() {
        if map.len() > MAX_METADATA_KEYS {
            return Err(format!(
                "edge metadata must have {MAX_METADATA_KEYS} keys or fewer"
            ));
        }
    }
    let metadata_bytes = metadata_byte_len(&normalized.metadata);
    if metadata_bytes > MAX_METADATA_BYTES {
        return Err(format!(
            "edge metadata must be {MAX_METADATA_BYTES} bytes or fewer"
        ));
    }
    if calculate_metadata_depth(&normalized.metadata) > MAX_METADATA_DEPTH {
        return Err(format!(
            "edge metadata must be nested {MAX_METADATA_DEPTH} levels deep or fewer"
        ));
    }
...
fn metadata_byte_len(value: &serde_json::Value) -> usize {
...
fn analyze(
    ...
    for node_edges in graph.values() {
        if node_edges.len() > MAX_EDGE_DEGREE {
            return Err(format!(
                "Refusing to analyze snapshot with a node having more than {MAX_EDGE_DEGREE} outgoing edges"
            ));
        }
    }
